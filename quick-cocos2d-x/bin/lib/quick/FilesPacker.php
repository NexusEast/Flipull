...
class FilesPacker
{
    ...
    protected function match_type($path,array $select_types)
    {   
    	  $is_match = false;
    	  foreach($select_types as &$current_type)
    	  {
    	  	//echo $current_type,"+++",$path,"+++",-strlen($current_type),"-------";
    	  	//echo substr($path,-strlen($current_type));
    	  	//echo "\n";
    	  if(substr($path,-strlen($current_type))==$current_type)
    	  {
    	  	//return false;
    	  	$is_match = true;
    	  	}
    ...
    protected function prepareForPack(array $files)
    {
        ...
        $dirDestRootName = $this->config['output'];
        $dirTmpRootName = $this->config['tmp'];
        #echo "srcPath=",$this->config['src'],"\n","destPath=",$this->config['output'],"\n","tmpPath=",$dirDestRootName,"\n";
        ...
        foreach ($files as $key => $path)
        {
            ...
            $fileNameBaseSrcDir = substr($path,strlen($this->config['srcpath'])+1);
            $pathDest = $dirDestRootName."/".$fileNameBaseSrcDir;
            $dirDestEntireName = dirname($pathDest);
            if (!file_exists($pathDest)) 
            {
            	if ( !file_exists($dirDestEntireName) || !is_dir($dirDestEntireName))
            	{ 
            		$this->createdir($dirDestEntireName,0777);
            		echo "******res_phone create dir=",$dirDestEntireName,"\n";
            	}
            	copy($path,$pathDest);
            	echo "******res_phone copy file create++++++++++++++++path=",$path,"\n";
            }
            
            $pathTmp = $dirTmpRootName."/".$fileNameBaseSrcDir;
            $dirTmpEntireName = dirname($pathTmp);
            if (file_exists($pathTmp))
            {
            	if(strcmp(md5_file($path),md5_file($pathTmp))!=0)
            	{
            		echo "copy md5 diff++++++++++++++++path=",$path,"\n";
            		copy($path,$pathTmp);
            	}
            	else
            	{
            		#echo "files-key-out:",$files[$key],"\n";
            		unset($files[$key]);
            		continue;
            	}
            }
            ...
            {
            	if ( !file_exists($dirTmpEntireName) || !is_dir($dirTmpEntireName))
            	{ 
            		$this->createdir($dirTmpEntireName,0777);
            		echo "create dir=",$dirTmpEntireName,"\n";
            	}
            	copy($path,$pathTmp);
            	echo "copy file create++++++++++++++++path=",$path,"\n";
            }
            
            ############################
            if($this->match_type($path,$select_types) == false)
            {
                unset($files[$key]);
            }
            else
