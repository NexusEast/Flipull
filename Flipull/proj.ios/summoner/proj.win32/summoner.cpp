    ...
    int       nCmdShow)
{
    ...
    return SummonerPlayer::createAndRun();
}

int SummonerPlayer::createAndRun(void)
{
    SummonerPlayer *host = SummonerPlayer::sharedInstance();
    int ret = host->run();
    SummonerPlayer::purgeSharedInstance();
    return ret;
}

SummonerPlayer *SummonerPlayer::s_sharedInstance = NULL;

SummonerPlayer *SummonerPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new SummonerPlayer();
    }
    return s_sharedInstance;
}

void SummonerPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
SummonerPlayer::SummonerPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int SummonerPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "summoner-player");
...
void SummonerPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void SummonerPlayer::createViewMenu(void)
{
...
void SummonerPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void SummonerPlayer::updateWindowTitle(void)
{
}

void SummonerPlayer::relaunch(void)
{
...
void SummonerPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void SummonerPlayer::onFileOpenProject(void)
{
...
void SummonerPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"SummonerPlayer");
...
void SummonerPlayer::onFileProjectConfig(void)
{
...
void SummonerPlayer::onFileRelaunch(void)
{
    relaunch();
}

void SummonerPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void SummonerPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void SummonerPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void SummonerPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void SummonerPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT SummonerPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    SummonerPlayer *host = SummonerPlayer::sharedInstance();
...
INT_PTR CALLBACK SummonerPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
