...
namespace dragonBones
{
    ...
    void Cocos2dxFactory::loadSkeletonFile(const String &skeletonFile
                                           , const String &name)
    {
...
#if defined(ENCRYPT_RESOURCE_XML) && ENCRYPT_RESOURCE_XML == 1
            unsigned long fileSize = 0;
            const char *pFileContent = (char *)cocos2d::CZHelperFunc::getFileData(skeletonFile.c_str() , "rb", &fileSize);
    ...
    void Cocos2dxFactory::loadTextureAtlasFile(const String &textureAtlasFile , const String &name, const String &PNGPath)
    {
...
#if defined(ENCRYPT_RESOURCE_XML) && ENCRYPT_RESOURCE_XML == 1
            unsigned long fileSize = 0;
            const char *pFileContent = (char *)cocos2d::CZHelperFunc::getFileData(textureAtlasFile.c_str() , "rb", &fileSize);
            doc.Parse(pFileContent);
#else
			unsigned long dummySize;
...
//                new_img_path
				doc.RootElement()->SetAttribute(ConstValues::A_IMAGE_PATH.c_str(), PNGPath.c_str());
			}
    ...
    /** @private */
    Object* Cocos2dxFactory::generateDisplay(cocos2d::ccBlendFunc blendFunc,ITextureAtlas *textureAtlas, const String &fullName, Number pivotX, Number pivotY)
    {
        ...
        // func.dst = GL_ONE_MINUS_SRC_ALPHA;
         atlasNode->setBlendFunc(blendFunc);
