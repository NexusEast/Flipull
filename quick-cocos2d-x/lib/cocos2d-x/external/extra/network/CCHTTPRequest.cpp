...
void CCHTTPRequest::setPOSTDataLua(CCArray*data)
{
    
    unsigned char *c = new unsigned char[data->count()];
    std::string str;
    for (int i= 0; i < data->count(); i++) {
        CCInteger *integer = (CCInteger*)data->objectAtIndex(i);
       // CCLog("integer[i]:%d=%02X",i,integer->getValue());
        c[i] = static_cast<unsigned char>(integer->getValue());
...
//       / CCLog("i:%d=%02X",i,c[i]);
    ...
    CCAssert(str.c_str(), "CCHTTPRequest::setPOSTData() - invalid post data");
    m_postFields.clear();
    curl_easy_setopt(m_curl, CURLOPT_POST, 1L);
    curl_easy_setopt(m_curl, CURLOPT_POSTFIELDSIZE,data->count());
    curl_easy_setopt(m_curl, CURLOPT_COPYPOSTFIELDS,c);
    delete c;
}
...
const string CCHTTPRequest::getResponseString(void)
{
    ...
    std::string temp =string(m_responseBuffer ? static_cast<char*>(m_responseBuffer) : "");
    
    return temp;
}

CCArray *CCHTTPRequest::getResponseByteArray()
{
    CCArray *ret = CCArray::create();
    unsigned char * c =( unsigned char*)m_responseBuffer;
    for (int i = 0; i < m_responseDataLength; i++) {
        
        ret->addObject(CCInteger::create((int)c[i]));
    }
    return ret;
}
